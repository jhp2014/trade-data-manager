import type { RankAxis, PlacedPoint, RankPoint, RankTarget } from "#domain";
...
export interface RankStore {
    /** 새 축 생성 → DB 가 부여한 id 를 채워 반환. */
    createAxis(name: string): Promise<RankAxis>;
    ...
    /** 배치 제거 — 비워진 slot 은 GC. 없는 배치는 조용한 no-op. */
