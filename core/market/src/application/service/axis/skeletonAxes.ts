...
//
// ## 3층 분리 — 변경이 어디서 멈추는가
//   앵커 행 → resolveSkeletons(해소) → skeletonShape(형태, 순수) → **여기(축)** → 값 하나
...
import { SKELETON_PARAM, skeletonShape, type ReviewPointKey, type SkeletonShape } from "#domain";
import { chartKeyOf } from "../shared/baselineResolver.js";
import { resolveSkeletons } from "../shared/skeletonResolver.js";
...
function skeletonAxis(spec: {
    ...
    return {
        ...
        inputs: ["adjDaily", "minute"],
        params: [SKELETON_PARAM],
 ...
 *
 * 네 개가 1턴차 사례들을 실제로 가른다(도메인 테스트로 고정):
