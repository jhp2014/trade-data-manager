// ⚠ **동결된 참조 구현 — 앱에서 쓰지 않는다.**(2026-09-18, 하루 우주 엔진으로 이주 완료)
// 살아 있는 엔진은 `domain/cellset/`(어휘·평가·시드)이고, 이 파일의 유일한 소비자는 이주 등가
// 게이트(`cellset/__tests__/equivalence.test.ts`)다 — 시드 조건 4칸이 옛 로직 4종과 같은 발화를
// 내는지 잠그는 자리. 그래서 배럴에서도 뺐다. 여기를 고칠 일은 없다(고쳐야 한다면 cellset 이다).
//
