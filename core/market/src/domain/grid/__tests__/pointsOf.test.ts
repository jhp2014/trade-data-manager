...
describe("pointsOf", () => {
    ...
    it("기준선 **정확 터치**(high == base)도 슬롯 2 를 연다 — 승격 자(strict >) 미달이라 문이 여기뿐(2026-09-10 대한광통신 코너)", () => {
        // 터치는 돌파(슬롯 1 breakout ≥ 유지)지만 그 고가는 레벨로 못 선다(승격은 strict >) — 옛 crossedTop(≥)은
        // 터치를 "상단 관통"으로 계산해 슬롯 2 를 닫았고, 재돌파가 양쪽 문 모두에서 새는 경계 구멍이었다.
        const g = grid({
            pivots: [hi(560, 10000, 570), lo(570, 9790)], // 눌림 −2.1% 가 터치 고가(=base)를 피벗으로 확정
            newHighs: [
                nh(560, 10000, 60), // 슬롯 1 — 정확 터치 breakout(≥), 승격 불가 고가 → 슬롯 2 개방
                nh(580, 10050, 40, true, 10000), // 슬롯 2 — 확정 고점 W(=base) 재돌파, 게이트 30 통과
            ],
        });
        const expected = [
            [560, "breakout", 0, 10000, null],
            [580, "renewal", 0, 10000, 560], // levelPrice = W = base 동가 — 슬롯 2 장부라 레벨 단조성과 무충돌
        ];
        expect(pointsOf(g).map((p) => [p.min, p.kind, p.levelIdx, p.levelPrice, p.levelMin])).toEqual(expected);
        // m'=0 에서도 동일 — "m'=0 슬롯 2 불가(v8 동치)"의 유일한 의도적 예외(pointsOf 머리 주석).
        expect(pointsOf(g, DEF0).map((p) => [p.min, p.kind, p.levelIdx, p.levelPrice, p.levelMin])).toEqual(expected);
    });

    it("연장이 정확 터치에 그치면 슬롯 2 는 안 닫힌다 — 소멸 판정도 승격 자(strict >)와 같은 자", () => {
        // 접근 슬롯 1(9,970) 뒤 미확정 연장이 base 를 정확 터치(10,000) — 그 고가도 레벨로 못 서므로
        // 소멸(레벨 완결) 아님. 눌림이 그 고가를 확정하면 재돌파 국면이 정상 발화한다.
        const g = grid({
            pivots: [hi(570, 10000, 585), lo(585, 9790)],
            newHighs: [
                nh(560, 9970, 60, true, 9800), // 슬롯 1 — 기준선 접근(밴드), 슬롯 2 개방(앵커 9,970)
                nh(570, 10000, 60, true, 9970), // 미확정 연장의 정확 터치 — 옛 규칙(≥)이면 여기서 영구 소멸
                nh(600, 10020, 40, true, 10000), // 슬롯 2 — 확정 고점 W=10,000 재돌파
            ],
        });
        expect(pointsOf(g).map((p) => [p.min, p.kind, p.levelPrice, p.levelMin])).toEqual([
            [560, "breakout", 10000, null],
            [600, "renewal", 10000, 570],
        ]);
    });

    it("정확 터치 후 strict 관통 연장은 여전히 소멸이다 — 재돌파는 그 고가의 새 레벨 슬롯 1 로 선다(손실 없음)", () => {
        // 터치 슬롯 1(10,000) → 미확정 연장이 10,050 으로 strict 관통(슬롯 2 영구 소멸) → 눌림이 10,050 을
        // 마디로 확정 → 재돌파는 슬롯 2 가 아니라 새 레벨의 슬롯 1 renewal — 관통 케이스의 경로는 안 바뀐다.
        const g = grid({
            pivots: [hi(570, 10050, 580, 570), lo(580, 9840)],
            newHighs: [
                nh(560, 10000, 60), // 슬롯 1 — 정확 터치, 슬롯 2 개방
                nh(570, 10050, 25, true, 10000), // strict 관통 연장(게이트 미달) — 슬롯 2 소멸
                nh(600, 10100, 40, true, 10050), // 새 레벨(10,050) 슬롯 1 renewal
            ],
        });
        expect(pointsOf(g).map((p) => [p.min, p.kind, p.levelIdx, p.levelPrice, p.levelMin])).toEqual([
            [560, "breakout", 0, 10000, null],
            [600, "renewal", 1, 10050, 570],
        ]);
    });
...
// ── 기준선 없는 격자(라벨만 있는 차트의 base null — 2026-09-23) ─────────────────────
// 격자가 기준선을 모르면 레벨 0 이 없고 **마디의 바닥도 없다**. 종단 소비자는 호출 전에 기준선 없는
// 격자를 거른다(defDerived 가드) — 이 절은 pointsOf 자체의 정의를 잠근다.
