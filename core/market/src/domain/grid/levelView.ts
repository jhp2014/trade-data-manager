...
// 최솟값(동가 tie 는 이른 봉 — 배열이 시간 오름차순이라 strict < 비교가 그 규칙). 항상 존재한다:
// 레벨은 터치 봉에서 확정되고 그 봉이 runLow 로 추적되어 확정되거나 꼬리로 남으며, 크로싱 봉에서는
// renew 규칙이 저점 확정을 강제한다(§2.2 dir=down 의 "항상 참"). 결손이면 throw — 침묵 오염 금지.
//
...
export function levelViewOf(grid: PointGrid): LevelPair[] {
    ...
    for (let k = 0; k < levelIdxs.length; k++) {
        ...
        if (lowIdx < 0) throw new Error(`levelViewOf: 레벨(min=${high.min}) 저점 결손(구간 끝 ${endMin}) — §2.5 불변식 위반`);
