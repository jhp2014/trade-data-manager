// core/market/domain/review — 타점 태그(사람 편집). 순서 없는 **명목형 분류**.
// 축(domain/rank)과 짝이자 대비: 축 = "상하 순서를 매길 수 있는 하나", 태그 = "순서가 없는 종류".
// 순서 없는 분류를 축에 배치했는지 여부로 표현하던 우회(있으면 A유형·없으면 B유형)를 대신한다.
//   · 한 타점에 여러 개(그게 명목형의 성질 — 단일 varchar 였던 reviewPoint.type 을 흡수).
//   · 사전(Tag)과 부착(TagAttachment) 분리 — 이름을 바꿔도 부착이 안 깨지고, 오타 난립을 사전이 막는다.
//   · 이름의 `그룹:값`(예: "형태:돌파")은 관례일 뿐 도메인 규칙이 아니다. 표시 색 그룹핑에만 쓴다.

/** 태그 사전 항목 1개(저장됨 → id 필수). name 은 전역 유일. */
export interface Tag {
    id: string;
    name: string;
}

/** 한 타점에 붙은 태그들(부착 피드 항목). 전 타점을 한 번에 받아 클라가 키로 접는다. */
