...
describe("DrizzleDailyCommentRepository (pglite)", () => {
    ...
    /** 읽기는 전량 하나(listAll) — 검증도 그 위에서 그 날만 거른다(소비자와 같은 모양). */
    const onDate = async (date: string): Promise<DailyComment[]> =>
        (await repo.listAll()).filter((r) => r.date === date).sort((a, b) => a.stockCode.localeCompare(b.stockCode));

    it("upsert 후 listAll 로 조회", async () => {
        await repo.upsert(dc({ stockCode: "005930", comment: "전공정 호조" }));
        await repo.upsert(dc({ stockCode: "000660", comment: "HBM 수급" }));
        const rows = await onDate("2026-06-30");
    ...
    it("upsert 재입력 — 같은 (date,stock) 은 comment·author 를 덮어씀(1행 유지)", async () => {
        ...
        const rows = (await onDate("2026-06-30")).filter((r) => r.stockCode === "005930");
    ...
    it("다른 날짜와 격리", async () => {
        ...
        expect((await onDate("2026-06-30")).map((r) => r.stockCode)).toEqual(["000660"]);
        expect((await onDate("2026-07-01")).map((r) => r.stockCode)).toEqual(["000660"]);
    });
});
