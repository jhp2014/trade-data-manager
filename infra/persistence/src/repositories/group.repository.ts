 ...
 *
 * DB 로는 못 막아 여기서 지키는 불변식: 부모 계층이 **순환하지 않는다**(순환하면 조회가 무한히 내려간다).
 * (옛 scope 검사 — 항목의 시각 유무가 그룹 층위와 맞나 — 는 2026-09-01 타점 층위 폐지로 사라졌다.)
 */
export class DrizzleGroupRepository implements GroupReader, GroupStore {
    ...
    async listAllMemberships(): Promise<GroupMembership[]> {
        ...
        for (const r of rows) {
            ...
            if (hit) hit.groupNames.push(r.groupName);
                ...
                byItem.set(key, { stockCode: r.stockCode, date: r.date, groupNames: [r.groupName] });
            }
        }
    ...
    async attach(groupName: string, item: GroupItemRef): Promise<void> {
        ...
        // 멱등 — uq_group_member_day 가 (그룹, 종목, 날짜)를 잡고 있어 충돌하면 그냥 넘어간다.
        await this.db
            .insert(groupMembers)
            .values({ groupId: id, stockCode: item.stockCode, tradeDate: item.date })
            .onConflictDoNothing();
    }
