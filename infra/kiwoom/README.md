...
```
pnpm --filter @trade-data-manager/kiwoom recon:token       # 모든 키 토큰 발급 확인
pnpm --filter @trade-data-manager/kiwoom recon:daily 005930 20260515
pnpm --filter @trade-data-manager/kiwoom recon:minute 005930 20260515
pnpm --filter @trade-data-manager/kiwoom recon:rotation    # 키 로테이션 분포 확인
pnpm --filter @trade-data-manager/kiwoom recon:pagination  # 연속조회 멀티페이지 + 키핀 검증
pnpm --filter @trade-data-manager/kiwoom recon:condition   # WS 조건검색(CNSRLST→CNSRREQ) [seq]
```
...
```
pnpm --filter @trade-data-manager/kiwoom test
```
