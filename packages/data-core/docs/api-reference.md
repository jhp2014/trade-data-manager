...
> ⚠️ **이동됨**: Sheet 표현 변환은 더 이상 data-core 가 제공하지 않습니다.
> `buildSheetMatrix` / `FIXED_COLUMNS` / `toManualHeader` 는 앱(chart-review)의 Sheet 계층
> (`apps/chart-review/src/lib/buildSheetMatrix.ts`, `sheetColumns.ts`)으로 옮겨졌습니다.
> data-core 는 `findReviewExportRows()` → `ReviewExportRow[]` 까지만 제공합니다.
> 피처 투영 목록 `FEATURE_COLUMNS` 는 `market-feature` 로 옮겨 read/export 공통으로 씁니다.
> 아래 설명은 과거 형태(앱 계층의 현재 구현과 동일)를 참고용으로 남깁니다.
...
| Sheet matrix 생성 | (앱) `apps/chart-review/src/lib/buildSheetMatrix.ts` |
