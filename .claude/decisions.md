...
- **테마 조건은 스냅샷 묶음 필터 하나다 — 테마 단위 AND, 테마 사이 ∃.** 타점 통과 ⟺ 소속 테마 중 묶음의 모든 하위 조건을 혼자서 다 만족하는 테마가 존재. 하위 조건 3종: 존 내 테마 종목 수 ≥x(자신 포함) · 테마 내 기본 순위(존 무관, 전 멤버 중) · 테마 내 존 순위(존에 든 멤버 중, 자신이 존 밖이면 불만족 — 결손은 결손). 순위 기준은 등락률 기본·거래대금 옵션. N/M 은 묶음의 정체성(동결), 임계값은 보드에서 인라인 조절. **하위 조건을 독립 레일로 분해하지 않는다** — 분해하면 테마 커플링이 풀려 다중 테마 종목에서 서로 다른 테마로 만족해도 통과한다(같은 N/M 을 두 번 스냅샷해 나누는 건 그걸 알고 쓰는 기여도 검사용 근사). 상시 단순 순위 레일(N/M 무관)은 만들지 않는다(사용자 확정 — N/M 맥락 없는 순위 필터는 쓰임 없음).

- **조건을 만드는 손은 편성 보드 하나 — 테마 순위 패널은 순수 시선이고, 보드가 풀(pull) 스냅샷을 뜬다.** 패널의 N/M/임계값 탐색 상태는 스토어 슬라이스(영속)라 패널이 닫혀 있어도 보드가 읽는다. 보드 테마 칸의 라이브 미러 항목이 현재 패널값을 상시 표시하다 클릭 순간 동결해 묶음 행을 만든다(안 보고 찍기 방지). 패널의 "흔적"은 별도 저장물이 아니라 깔때기에 걸린 테마 필터의 파생 뷰(클릭 = 탐색값 복원). 기각: 패널 쪽 "필터에 추가" 푸시 — 조건 거는 장소가 패널마다 늘어난다(축 서랍이 팝오버를 기각한 사유와 동일).

- **보드 "테마" 칸은 UI 그룹핑이지 새 grain 이 아니다.** 테마 값들의 행 정체성은 전부 타점 — grain 4레이어 규칙(위 축 시스템 항목)을 건드리지 않는다. 확장 문법: **1차원 축 = 보드 레일 / 다차원·탐색형 조건 = 전용 패널 + 보드 스냅샷**(맵의 깔때기 구독 문법과 동렬). 기각: 계산축 레일을 축마다 전용 패널로 분해하고 보드를 목록+순서만 남기는 안 — 레일의 존재 이유(분포를 보면서 자르기)와 순서에 따른 한계기여도가 흩어진 패널에서는 성립하지 않고, 붐빔은 dockview 레이아웃으로 이사할 뿐이다(보드 붐빔의 처방은 이미 축 서랍). 클라 파생 값의 시트 열 침투는 보류.

- **테마 강도 술어의 동결 파라미터는 술어 payload 안에 산다.** `SavedSet` 은 `stages` 를 통째 복사하므로 params 를 슬라이스 참조로 두는 순간 집합의 자립이 깨진다(그 집합을 다른 데서 열면 남의 탐색값으로 평가된다). 필터 grain 은 `"point"` 고정이고, 단면 없음(pending·미수집·시각 없는 후보 하루)·재료 미도착은 **결손(미배치 칸)** 이지 탈락이 아니다(3치). 파서는 `lib/themeStrength.parseThemeStrengthParams` 한 벌 — 슬라이스와 술어가 같은 유효성 정의를 본다(관대한 병합: 필드 증설이 옛 저장물을 통째 죽이지 않게).
