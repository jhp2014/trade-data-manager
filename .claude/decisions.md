...
## 테마 강도·순위 단면 (2026-08-28 설계 확정, 구현 전 — 전체 서사 [[theme-strength-rank-design]])

- **테마 강도의 저장물은 N/M 을 모른다 — (날짜,분) 단면에 그날 유니버스 전 종목의 등락률·거래대금 서수 전체를 굽는다.** 타점이 존재하는 분만(같은 분 타점 여럿 = 단면 하나 공유), apps/api 신규 서비스가 DerivedCache(day 스냅샷)+큐레이션 타점을 주입받아 계산·날짜별 파일 영속, 엔드포인트 1개로 클라가 통째 로드(수 MB) — 테마·N/M·임계값은 전부 클라 읽기 시점 파생. 순위 규칙: UN 기준, 그 분 결손 종목 제외(분모 축소), 복기보드 `selectHotUniverse` 와 같은 정렬. 관리는 이벤트 훅이 아니라 **대사(reconcile)**: 기대집합(현재 타점들) vs 저장집합, 빠진 단면 계산·참조 없는 단면 GC(게을러도 무해).
