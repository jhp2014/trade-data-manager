...
- **슬롯 2 모델**: 레벨당 Point 최대 2개. 슬롯 1 = 레벨 밴드 첫 자격 사건 캔들(kind·게이트 기존). 슬롯 2(재돌파, 게이트 30억 공용) = 슬롯 1 고가가 레벨로 승격 불가능(strict > 미달, high ≤ L — 기준선·마디 공통, 2026-09-10 통일: 옛 "기준선 high < base" 는 정확 터치가 새는 구멍이었다)일 때만, 워터마크(슬롯 1 고가)를 **실초과**하는 다음 자격 사건 캔들 — levelPrice/levelMin = 워터마크 가격/봉. 눌림 조건 없음(2% 미만 눌림은 격자 해상도 밖). 귀속이 위 레벨로 점프하면 슬롯 2 소멸. "kind = levelIdx 0 파생" 정리 폐기(kind = 슬롯의 함수).
...
- **m'=0 완전 불변**(recon 실측): 갈림 0 · 슬롯 2 발화 0 · 무터치 Point 0 — 상단 미달 슬롯 1 이 m'=0 에선 원리적으로 불가능. ⚠ 2026-09-10 개정: crossedTop 승격 자 통일(strict >)로 **기준선 정확 터치(high == base)만 예외** — m'=0 에서도 슬롯 2 가 열린다(decisions.md 「자동 타점 격자」 crossedTop 항목).
- **실측(0.5, 6,016차트)**: Point 9,869 → 12,315(+2,446). 슬롯 2 1,819(**상단 관통 81.7%** — "다음엔 상단도 자연히 넘는다" 가설 확인) · 훼손→재돌파 간격 p50 2분/p90 23분 · ≤1분(연속 상승 코너) 845. touch 완화 신설 = 무터치 날 Point 115(100일) + 터치 이전 접근 Point 821. naive 재진술(슬롯 맵 형태) 6,016 전량 일치.
- 기각: 눌림 최소폭 조건(굽지 않은 값이라 정의 불가), 슬롯 무한 반복(0.5% 안 통계 오염), 시간 간격 노브(연속 상승 코너 665건 실측 후 필요하면 additive).

## 12. 후속 확정(2026-09-06) — 슬롯 2 눌림 확정 요건

사용자 차트 관찰(현대로템 2025-08-05 15:03/15:04 인접 쌍 — "하나만 나와야")에서 출발. §11 의 "눌림 조건 없음"과 그 기각 사유("굽지 않은 값이라 정의 불가")를 **뒤집는다** — 오진이었다: 요구가 정확히 ≥2%(격자 해상도 = 피벗)라 완전 판독 가능.

- **재돌파 정의 확정판**: "눌림(zigzag 2%)으로 **확정된** 고점을 다시 넘음. 넘음 = 그 고점의 밴드 하단(×(1−m'/100))을 넘음."
- **발화 3요건**: ① 확정 고점 W 존재 = 슬롯 1 이후(피벗 min ≥ 슬롯 1 봉·가격 ≥ 슬롯 1 고가) 캔들 **이전에 확정된**(confirmedMin strict) 고점 피벗의 최대(`confirmedHighSince`) ② 캔들 고가 > W×(1−m') ③ 게이트 30억. levelPrice/levelMin = W 가격/피벗 봉.
- **눌림 미확정 재상승 = 같은 돌파의 연장**: Point 아님(후보로만 남음 — 게이트 상향 시 슬롯 1 이동 의미론 유지). 연장이 상단 관통하면 슬롯 2 영구 소멸(그 고가가 눌림 후 새 마디 → 진짜 재돌파는 새 레벨 슬롯 1, 손실 없음).
- **게이트 미달 캔들은 자도 국면도 안 바꾼다**(첫 통과 승리 — 슬롯 1 과 같은 문법). 미확정 고가는 자가 아니므로 "자 상승 + 눌림 재요구" 비대칭이 원천 차단된다.
- **워터마크 상태 폐지**(§11 의 고정/트레일링 논쟁 통째 소멸): 자를 피벗에서 그때그때 읽는다. 피벗은 볼륨 무관이라 저대금 연장이 세운 고가도 눌림 확정 시 자로 선다(격자 밴드 상태의 볼륨 무관과 같은 원칙).
- **m'=0 완전 불변 유지**(recon: 갈림 0·슬롯 2 0). 전부 읽기 층, 재굽기 없음. ⚠ 2026-09-10 이후 기준선 정확 터치 슬롯 2 만 예외(위 §11 주석).
- **실측(0.5, 6,016차트)**: Point 12,315 → 10,759. 슬롯 2 1,819 → **263**(−1,556 — ≤1분 연속 상승 코너 845 전멸 포함) · 확정 고점→재돌파 간격 p50 **17분**/p90 117분/max 584분 · **≤1분 0**(눌림 요건의 구조적 보장 — recon 트립와이어로 상설) · 상단 관통 79.1%. naive 재진술(슬롯 맵 + filter/reduce W) 6,016 전량 일치.
- 기각: 시간 간격 노브(임의 상수 — 눌림 요건이 대체·§11 유보 해소), 눌림 크레딧 리셋(게이트 미달이 국면을 닫는 비대칭).
