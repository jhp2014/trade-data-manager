...
- **infra/persistence/src** — Drizzle 기반 core 리포지토리 포트 구현(전용 `market` 스키마): `Drizzle*Repository`(DailyCandle/RawDailyCandle/MinuteCandle/StockMaster/DailyMarketCap/StockNews/DailyComment/ChartAnchor/ReviewPoint/Group) + `DrizzleDailyUniverseProvider`, `createDb`, `syncCurationMirror`, `mappers/*`
