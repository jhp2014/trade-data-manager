...
- 개발 서버 기동: `$env:API_PORT=3011; pnpm --filter @trade-data-manager/api dev` · `pnpm --filter @trade-data-manager/workbench dev -- --port 3110`.
  루트 `pnpm dev` 는 3001/3100 을 뺏으므로 **개발 워크트리에서 금지**. 같은 이유로 `.claude/launch.json` 에는 **개발 항목(api-wt·workbench-wt)만 둔다** — 사용자 포트를 띄울 손잡이를 아예 안 만드는 것이 경고문보다 확실하다(사용자 화면을 봐야 하면 `preview_start` 에 url 을 직접 준다). apps/live 는 양쪽 다 VPS(100.74.165.85:3002)를 본다.
