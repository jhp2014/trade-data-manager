...
// 격자 = 앵커 차트(종목,날짜)당 ① 2% 양방향 zigzag 피벗(경로 뷰 — 마디 뷰는 클라가 core `levelViewOf` 로
// 파생, 레벨 고점엔 직전 레벨의 크로싱 봉) ② floor(20억) 이상 대금의 기준 밴드 사건 캔들 목록(newHighs —
// 봉마다 maxBefore(직전 러닝 최고가), `high > maxBefore` 가 상단 돌파) ③ 기준선 첫 터치 봉 — 기록 봉마다
// 세션 누적 대금(cum)을 들고, 대금 창(leg·갱신·돌파)은 굽지 않고 클라가 두 봉의 차로 파생한다
// (core `windows.ts`). Point 판정(게이트·자격 시각 창·축약 병합)은 core `pointsOf` 가 유일한 계산 주체고
...
//
// version 은 검출 규칙 버전(core POINT_GRID_RULE_VERSION = 서버 POINT_GRID_CALC_VERSION) — 클라 디코더
// (fetchPointGrids)가 불일치 시 throw 한다(튜플 칸 수가 같은 "의미만 바뀐 번들"의 침묵 오독 차단).
// 재료 수리는 자동 감지되지 않으므로 이 값 상향(또는 캐시 삭제)이 처방이다(순위 단면과 같은 성질).
