...
`Result<T>`는 **Server Action 경계에서만** 사용한다. 내부 함수(레포지토리, 어셈블러,
도메인 서비스, 로더 등)는 일반적인 `throw`를 사용하고, Server Action이 `try/catch`
로 받아서 `errResult(...)`로 감싼다.

이유:
- 내부 호출 체인까지 `Result`를 흘려보내면 각 호출부에서 `if (!r.ok) return r` 보일러
  플레이트가 누적된다. throw는 컴파일러가 흐름을 자동으로 끊어준다.
- 직렬화/사용자 메시지 변환은 클라이언트로 결과가 넘어가는 경계에서만 필요하다.
  내부 에러는 stack trace를 잃지 않고 위로 던지는 게 디버깅에 유리하다.

따라서 다음 같은 코드는 의도된 정상 패턴이다:

- `getThemeBundle`, `loadDecksFromDir`, `marketRepository.*` 등은 `throw`
- `loadDeckAction`, `fetchChartPreviewAction` 등 `"use server"` 함수가 `try/catch` 후
  `okResult(...)` / `errResult(...)` 로 감싸서 반환
