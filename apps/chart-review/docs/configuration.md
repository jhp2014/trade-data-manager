...
| `CHART_PARAMS_DEBOUNCE_MS` | 200 | a/d 빠른 탐색 시 차트 fetch 디바운스 |
...
| 상수/함수 | 의미 |
|-----------|------|
| `VIEW_MODES` / `VIEW_MODE_CYCLE` | 중앙 차트 뷰 순서·라벨(헤더 세그먼트 + e/q 순환의 단일 출처) |
| `cycleViewMode(cur, ±1)` | 현재 뷰에서 한 칸 순환 |
| `SHORTCUT_KEYS` | 전역 단축키 매핑(a/d/w/s/e/q/Space) |
