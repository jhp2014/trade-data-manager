...
### 메인 차트 무요청화 → (a) 채택·구현 완료

테마 멤버 클릭 시 메인 daily/minute 차트를 **번들에 이미 있는 `member.daily/minute`
로 그려 완전 무요청**(a). 구현:
- 번들 fetch 를 **앵커(anchorStock)** 기준으로 키잉. 테마 peer 탐색은 앵커를
  유지해 재요청 없음. 작업셋 밖(붙여넣기/히스토리)만 앵커를 옮겨 새 번들을 받는다.
- 메인차트 데이터(`mainChartData`)는 override + activeReview 면 멤버 raw
  (daily/minute) + entryCandle 파생 prevClose 로 구성, 아니면 앵커 self 그대로.

트레이드오프(수용): 무요청 peer 탐색 중 사이드바 테마 리스트는 **앵커 종목의
테마에 고정**. 테마 맥락 전환은 붙여넣기/히스토리(=재요청)로만. (사용자 확정:
메인 종목 두고 테마 탐색이 주 목적이라 OK.)

Point List/라인 무요청화는 위 DTO enrich 로 (a) 와 함께 달성됨.
...
- **S5** ✅ 메인차트 무요청화(§2 스코프 a): 번들 fetch 를 앵커 기준 키잉 +
  메인차트를 멤버 raw(daily/minute/prevClose)로 렌더. 테마 peer 탐색 무요청.
