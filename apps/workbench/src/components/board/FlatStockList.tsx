...
// flat 리스트 — 거래대금순 StockRow. 실시간 테마·테마[장 마감]·[복기] 보드 공용(테마 그룹 대신 flat 뷰).
export function FlatStockList({ stocks, code, onPick, empty = "종목 없음" }: {
    stocks: BoardStock[];
    code: string;
    onPick: (code: string) => void;
    empty?: string;
}): JSX.Element {
    const rows = useMemo(() => [...stocks].sort((a, b) => b.amount - a.amount), [stocks]);
