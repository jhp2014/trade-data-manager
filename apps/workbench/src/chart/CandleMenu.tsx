...
//
// **props 는 관심사 그룹**(lines·ignore)이다 — 평평한 17개였을 때 param 이
...
/** 필드 버튼 한 칸의 표시 — 상태(disabled)·타이틀을 cellOf 로 주입하는 줄 계약. */
interface FieldCell {
    disabled?: boolean;
    title: string;
    /** 값 자리 표시 override. 기본 = 현재 시장의 캔들 값. */
    text?: string;
}

/**
 * 시고저종 버튼 한 줄 — 라벨 + 필드 4버튼. 상태(on/disabled)·타이틀은 cellOf 로 주입한다.
 * (옛 골격 두 섹션도 이 줄을 썼다 — param 이 늘면 복붙이 될 자리를 여기 하나로 고정한다.)
 */
