// 얹는 선(기준선·D선) 층 — 골격선과 **같은 pct 환산**으로 가격 수준선을 얹는다.
//
// **주인이 스타일을 정한다**(사용자 확정): 색은 그 골격선과 똑같이(visualOf 결과를 owner 로 받는다) —
// 그룹 목록을 훑을 때 골격선은 무리 색인데 가로선만 앰버로 뜨면 "이게 어느 골격의 선이냐"를 다시
...
import { pct, type NormalizedSkeleton } from "./skeletonOverlay.js";
...
export function LevelsLayer({ owners, levelsOf, scaleY, box }: {
    ...
    levelsOf: (chartKey: string) => readonly SkeletonWireLevel[];
