...
//
// ## 왜 있나
// 이 패널의 순수 그림 층(캔들·테마 선·골격선)은 팬 한 프레임마다 통째로 다시 그려진다. 지금은 그게
// JSX 라서 프레임마다 React 재조정 + DOM 쓰기가 따라붙는데, 실측으로 **비용을 정하는 건 점 수가 아니라
// DOM 노드 수**였다(panCost 벤치: 100선×60피벗 200노드 6000점 = 24ms vs 100선×8피벗 1000노드 800점 = 42ms).
...
/**
 * 그림 층의 **그리는 순서** — 먼저가 아래. 이 패널에서 순서는 미학이 아니라 뜻이다:
 *   · 캔들이 맨 아래 — 골격이 그 위를 지나야 "축약이 원본의 어디를 밟았나"가 읽힌다.
 *   · 테마 선이 골격선보다 아래 — 배경이고 주인공은 내 골격이다.
 *
 * **세 층은 반드시 붙어 있어야 한다.** 캔버스 한 장은 스택에서 자리를 하나만 차지하므로,
 * 사이에 DOM 층(손짓)이 끼면 캔버스로 옮길 때 순서를 재현할 수가 없다 — 그래서 손짓 층
 * (theme-hit)은 이 뭉치 **뒤로** 나가 있다. 그림 층은 포인터를 안 받으니 손짓 우선순위는 안 바뀐다.
 */
export const PAINT_ORDER = ["candles", "theme-lines", "skeleton-lines"] as const;
export type PaintName = typeof PAINT_ORDER[number];

/**
 * 이름표 붙은 층들을 **정해진 순서로** 늘어놓는다 — 부르는 자리가 순서를 못 정하게 한다.
 * 순서가 한 상수에만 있으니 SVG 페인터든 캔버스 페인터든 같은 줄을 선다.
 */
