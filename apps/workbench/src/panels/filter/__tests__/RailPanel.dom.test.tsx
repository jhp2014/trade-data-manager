// 필터 레일 패널 — 1차원 조건을 긋는 판. **레일 하나 = 필터 하나**가 이 화면의 규칙 전부다.
//
...
//   조건을 긋게 된다.
//
// ⚠ 이 판은 제 상태가 없다 — 컷은 곧바로 store 의 필터다(집합 편성 보드와 같은 저장소, 사본 없음).
// 그래서 검사도 화면이 아니라 **store 의 stages** 를 본다.
...
const renderRails = (seed: Seed = SEED): ReturnType<typeof render> =>
    render(<RailPanel panelId={PANEL} />, {
        wrapper: ({ children }: { children: ReactNode }) => <Providers client={seededClient(seed)}>{children}</Providers>,
    });

/** "걸린 것만"은 패널 로컬 영속 상태다(머리글 토글) — 검사는 그 자리를 직접 심는다. */
const setOnlyActive = (on: boolean): void => {
    useWorkbench.setState({ panelUi: { [PANEL]: { railsOnlyActive: on } } });
};
...
describe("사전이 오기 전 — 빈 레일을 그리지 않는다", () => {
    it("불러오는 중이라고 말한다 — 빈 레일은 '축이 없다'는 거짓말이다", () => {
        ...
        try {
            const { container } = render(<RailPanel panelId={PANEL} />, {
...
describe("층위 칸 — 무엇이 어디에 사나", () => {
    ...
    it("값이 없는 축은 트랙 대신 이유를 적는다 — 빈 트랙을 주면 그을 수 있는 줄 안다", () => {
        const { container } = renderRails({ ...SEED, computedAxes: feeds.map((f) => ({ ...f, values: [] })) });
...
// ⚠ 이 블록이 이 파일의 존재 이유 둘째다 — 깔때기 직결(사본 없음)의 배선.
...
describe("서랍 — 축을 치우되 조건은 살려 둔다", () => {
    ...
    it("접힌 서랍의 조건을 되짚으면 서랍이 펼쳐진다 — 안 그러면 눌러도 아무 일이 없다", () => {
        ...
        try {
            ...
            // 되짚기 신호는 보드에서 온다 — 패널 경계를 넘으므로 세션 자리에 남는다.
...
// 서브 띠(갱신/고점)는 2026-09-04 고점 축 은퇴와 함께 소멸 — 시그널 이후 값은 결과 패널이 진다(decisions "시그널 결과").
