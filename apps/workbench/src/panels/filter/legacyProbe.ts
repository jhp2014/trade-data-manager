...
//
// 그래서 **한 번도 조건을 저장한 적 없는 사용자의 앱 시작 때 한 번만**, 그 가방을 읽어 조건 칸으로
// 세운다(입구 하나 — 2026-09-19 9단계로 "우주 전환" 이라는 입구 자체가 사라졌다: 우주는 조건에서
// 파생되고, 여기서 심는 조건들이 하루 전용이라 심는 순간 우주가 하루가 된다).
//  · 한 번뿐인 이유: 매번 심으면 **사용자가 지운 칸이 재시작마다 되살아난다**("내가 지운 게 돌아왔다").
//    도장(MIGRATED_KEY)이 그 한 번을 지킨다.
