...
/** 우선순위 — 클수록 강하게 묶인다. 잎은 원자라 가장 강하다. */
const prec = (e: SetExpr): number => (e.kind === "cond" ? 3 : e.kind === "and" ? 2 : 1);
 ...
 */
export function renderExpr(e: SetExpr, labelOf: (id: string) => string): ExprPiece[] {
    const out: ExprPiece[] = [];

    const walk = (n: SetExpr, parentPrec: number): void => {
        const neg = n.neg === true;
        // 괄호가 필요한가 — ① 부모보다 약한 묶음이거나 ② 부정된 묶음(¬ 가 첫 항만 먹지 않게).
        const needParen = n.kind !== "cond" && (prec(n) < parentPrec || neg);
