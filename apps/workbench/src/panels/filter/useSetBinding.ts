...
// 구조의 해체: 연동(null)이 디폴트라 아무것도 안 만지면 오늘과 같고, 고정하고 싶은 패널만 참조를 묶는다.
// "연동은 따라가고, 고정은 안 따라간다" — HTS 연동 채널과 같은 문법.
//
...
export interface SetBinding {
    /** null = 연동(디폴트) — 깔때기의 "보는 집합"(짚은 칸 반영, 없으면 최종 생존)을 따라간다. */
    ref: SetRef | null;
...
export function setRefLabel(ref: SetRef, savedFunnels: readonly SavedFunnel[]): string {
    switch (ref.kind) {
        ...
        case "filter": return ref.filterId === null ? "활성 필터" : (savedFunnels.find((f) => f.id === ref.filterId)?.name ?? "(지워진 필터)");
        case "cell": {
            const base = ref.filterId === null ? "활성 필터" : (savedFunnels.find((f) => f.id === ref.filterId)?.name ?? "(지워진 필터)");
            return `${base} · 칸`;
        }
...
export function useSetBinding(storageKey: string): SetBinding {
    ...
    return {
        ...
        setRef,
        view,
        label: ref === null ? "연동" : setRefLabel(ref, savedFunnels),
        broken: view.broken,
    };
}
