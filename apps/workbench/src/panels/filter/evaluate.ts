...
// 결손으로 세어져 "안 맞았다"가 아니라 "아직 안 했다"가 된다. 이게 3치를 쓰는 이유 전부다.
//
...
export interface EvalLookup {
     ...
     * 위의 groupNamesOf(조상까지 편 집합)로는 못 묻는다 — 조상은 개수를 안 바꾸지만 이름은 늘린다.
     */
    anyGroupAt: (item: FunnelItem) => boolean;
 ...
 * 그 조건 자체가 뜻을 잃었으므로 "안 맞았다"로 세면 결손이 탈락으로 새어 나간다.
 */
export function evalGroupExpr3(expr: GroupExpr, item: FunnelItem, look: EvalLookup): Verdict {
    ...
    const holds = (groupId: string): Verdict => {
        // "그룹 없음"은 그룹이 아니라 **개수 조건**이라 사전을 안 본다(죽을 수가 없다).
        if (isNoneLiteral(groupId)) return not3(look.anyGroupAt(item));
...
export function evalPredicate3(p: FilterPredicate, item: FunnelItem, look: EvalLookup): Verdict {
    ...
    switch (p.kind) {
        ...
        case "group": {
            ...
            // 그 **하루**에 묻는다 — 깔때기 해상도가 타점으로 내려가도(다른 point 조건 탓) 하루 답이
            // 그날 타점 전부에 적용된다(층위 상속과 같은 방향). 안 벗기면 grain 이 내려가는 순간 뜻이
            // "이 타점 자신이 라벨됨"으로 조용히 바뀐다. ⚠ 팔레트는 B안(1:1)으로 point 그룹을 day
            // 조건에 새로 못 넣지만, **승계 저장물**(scope 부재=day)의 point 그룹 리터럴이 살아 있어
            // 이 시각 벗김(∃ 상향 포함)은 평가 계약으로 남는다 — 입구를 닫았다고 여길 걷어내면 옛
            // 저장물의 뜻이 바뀐다.
