// 교집합 노드를 두 노드 **사이 어디에** 놓을까(순수) — 정확히 한가운데일 필요는 없다.
//
// 한 자리에서 여러 갈래가 나가면 중점들이 한 골목에 몰려 교집합 노드끼리 겹친다. 그래서 노드를
// **제 선 위에서 앞뒤로만** 미끄러뜨려 떼어 놓는다:
//   · 움직임을 선 방향 1차원으로 가두면 노드가 제 선에서 안 떨어진다(어느 짝의 교집합인지가 안 흐려진다).
...
/** 이보다 가까우면 겹친 것으로 본다(교집합 노드 한 칸 + 여백). */
const MIN_DIST = 56;
...
/**
 * 교집합 노드 자리 — 서로 `MIN_DIST` 만큼 떨어지도록 각자 제 선 위에서 미끄러뜨린다.
 * 완전 해소를 보장하지 않는다(선이 셋 이상 한 점에 몰리면 물리적으로 불가능).
 */
export function spreadMidpoints(specs: readonly MidpointSpec[]): Map<string, Point> {
