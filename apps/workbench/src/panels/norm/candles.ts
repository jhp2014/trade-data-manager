...
import { fillGaps, pct } from "./overlay.js";
...
/**
 * 일봉 캔들 — 분봉과 **x 규칙만 다르다**: x 는 벽시계가 아니라 **배열 인덱스 − baseT**(거래일 오프셋,
 * baseT = D 의 인덱스라 D 가 0). basePrice 는 D−1 종가(전일 종가 원점 — 사용자 확정).
 */
export function dailyOverlayCandles(
    bars: readonly RawDaily[],
    origin: { basePrice: number; baseT: number },
    /** 어느 시장의 봉을 그리나 — 원점(전일 종가)과 **함께 갈리는** 패널 토글(사용자 확정). */
    market: "krx" | "un",
): ViewCandle[] {
