...
describe("dailyOverlayCandles — 일봉 → 뷰 공간(x = 창 안 거래일 순번)", () => {
    ...
    it("배열 인덱스가 곧 t — x 는 baseT(D 인덱스) 기준 상대 거래일", () => {
        const out = dailyOverlayCandles(bars, { basePrice: 120, baseT: 2 }, "un");
        expect(out.map((k) => k.x)).toEqual([-2, -1, 0, 1]);
    });

    it("y 는 원점 가격 대비 % — 원점 가격의 봉 종가가 0% 에 앉는다", () => {
        const out = dailyOverlayCandles(bars, { basePrice: 120, baseT: 2 }, "un");
    ...
    it("거래대금은 **실측 그대로** — 일봉은 OHLC×량으로 지어내지 않는다", () => {
        ...
        expect(dailyOverlayCandles(withAmt, { basePrice: 92, baseT: 0 }, "un")[0].amount).toBe(12_300_000_000);
    });

    it("시장은 **패널 토글이 명시**한다 — 봉과 원점(전일 종가)이 함께 갈린다(옛 holds 휴리스틱 폐기)", () => {
        const split: RawDaily[] = [{
            krx: { open: "100", high: "130", low: "90", close: "120", amount: "0" },
            un: { open: "205", high: "210", low: "200", close: "208", amount: "0" },
        }];
        expect(dailyOverlayCandles(split, { basePrice: 120, baseT: 0 }, "krx")[0].c).toBeCloseTo(0);
        expect(dailyOverlayCandles(split, { basePrice: 208, baseT: 0 }, "un")[0].c).toBeCloseTo(0);
    });

    it("값이 하나라도 가격이 아니면 그 봉은 건너뛴다 — 반쪽 캔들은 지어낸 그림이다", () => {
        const broken: RawDaily[] = [d(90, 95, 88, 92), { krx: d(0, 0, 0, 0).krx, un: { open: "", high: "9", low: "8", close: "9", amount: "0" } }];
        expect(dailyOverlayCandles(broken, { basePrice: 92, baseT: 0 }, "un")).toHaveLength(1);
    });

    it("기준가가 없거나 봉이 없으면 빈 목록", () => {
        expect(dailyOverlayCandles(bars, { basePrice: 0, baseT: 0 }, "un")).toEqual([]);
        expect(dailyOverlayCandles([], { basePrice: 100, baseT: 0 }, "un")).toEqual([]);
    });
});
...
describe("dailyOverlayCandles — 고가 등락률(봉 위 마커의 재료)", () => {
    ...
    it("직전 봉 종가 대비 고가 % — 차트 패널 마커와 같은 값이라 두 화면의 숫자가 안 갈린다", () => {
        ...
        const out = dailyOverlayCandles([d2(95, 101, 94, 100), d2(102, 130, 100, 128)], { basePrice: 100, baseT: 0 }, "un");
    ...
    it("직전 종가가 가격이 아니면 없다(0으로 나누지 않는다)", () => {
        ...
        expect(dailyOverlayCandles(bad, { basePrice: 2, baseT: 1 }, "un")[0].highPct).toBeUndefined();
    });

    it("하락한 날은 음수 — 임계 판정은 색 함수(highMarkerColor)의 몫이라 여기선 값만 낸다", () => {
        const out = dailyOverlayCandles([d2(95, 101, 94, 100), d2(90, 95, 88, 89)], { basePrice: 100, baseT: 0 }, "un");
