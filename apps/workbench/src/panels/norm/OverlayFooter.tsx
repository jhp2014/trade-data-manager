// 정규화 겹치기의 **발끝 줄** — 지금 무엇을 보고 있고 손으로 무엇을 할 수 있나.
//
...
export function OverlayFooter({ grain, groupNames, themeMode, themeLineCount, candles, amountWidthOn }: {
    ...
    /** 켜 둔 테마 멤버 캔들 — 이름을 다 적어 어느 종목을 보고 있는지 남긴다. */
    candles: { names: string[]; onClear: () => void };
        ...
        <div style={footer}>
            ...
            {candles.names.length > 0 && (
                <span style={{ color: "var(--text-secondary)" }}>
                    {" · 멤버 캔들 "}
                    {candles.names.join("·")}
                    <button onClick={candles.onClear} title="켜 둔 멤버 캔들 전부 끄기" style={footerBtn}>✕</button>
                </span>
            )}
            {themeLineCount > 0 && (
