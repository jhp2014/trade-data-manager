...
import { candlePrice, beatsAsBaseline, anchorCoordKey, chartAnchorKey } from "@trade-data-manager/market/domain";
...
import { buildMarks, displayOf } from "../../lib/anchorMarks.js";
import { toNormMarks, type NormMark } from "./anchorDisplay.js";
...
export interface NormItemsView {
    ...
    /** 차트키 → 상단 표식(이 패널 grain 의 앵커만) — 표식 층이 그린다. 표기 규칙은 lib/anchorMarks. */
...
export function useNormLines(grain: "daily" | "minute", dailyMarket: "krx" | "un", zeroLine: ZeroLine): NormItemsView {
    ...
    // param 필터는 표기 레지스트리(lib/anchorMarks)가 진다 — 기준선 하나가 아니라 등록된 전부.
    ...
    return useMemo<NormItemsView>(() => {
        ...
        for (const it of items) {
            ...
            if (!levelsByChart.has(ck)) {
                ...
                const { levels, winnerKey } = levelsOf(bundle, mine, isDaily ? 1 : (bundle.rawScale ?? 1));
                ...
                marksByChart.set(ck, toNormMarks(buildMarks(mine, { minutePanel: !isDaily, winnerKey }), {
                    minute: !isDaily,
...
function levelsOf(
    ...
    viewScale: number,
): { levels: NormLevel[]; winnerKey: string | null } {
    const rawScale = bundle.rawScale ?? 1; // 없으면(옛 서버) 1 — 계수가 없던 시절의 동작
    const resolved: { price: number; coord: string; key: string; minute: boolean }[] = [];
    for (const a of anchors) {
        ...
        resolved.push({ price: adj * viewScale, coord: anchorCoordKey(a), key: chartAnchorKey(a), minute: a.anchorTime != null });
    }
    if (resolved.length === 0) return { levels: [], winnerKey: null };
