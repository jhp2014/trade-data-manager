// Daily 패널 주소 — 여러 자리(작업 대상·테마 판·생성소·격자판)가 가리키는 id 를 잎 모듈 하나에 둔다.
//
// 생성소는 옛 「집합 편성」의 **idBase·component 를 승계**한다(`filter-funnel` / `filterFunnel`) —
...
/** 격자판 밑동 — 생성소의 「돌파」 줄이 연동하는 판(인스턴스는 연동 맵이 가리킨다). */
