...
export function SavedFilterBar({ axes }: { axes: { id: string; name: string }[] }): JSX.Element {
    ...
    return (
        <div style={{ flexShrink: 0, display: "flex", alignItems: "center", gap: 6, padding: "5px 10px", borderBottom: "1px solid var(--border-subtle)", background: "var(--bg-secondary)", flexWrap: "wrap", minHeight: 30 }}>
            ...
            {saved.length === 0 && <span style={{ fontSize: 11, color: "var(--text-tertiary)" }}>밴드 걸고 "현재 저장" → 칩 클릭으로 불러와 비교</span>}
            {saved.map((f) => (
            ...
            <button onClick={() => bandsActive && saveFilter(autoLabel(), rankBands)} disabled={!bandsActive} title={bandsActive ? "현재 밴드를 저장 필터로 담기" : "먼저 밴드를 거세요"}
                style={{ ...miniBtn, marginLeft: 4, opacity: bandsActive ? 1 : 0.45, cursor: bandsActive ? "pointer" : "default", borderStyle: "dashed" }}>+ 현재 저장</button>
        </div>
    );
}
