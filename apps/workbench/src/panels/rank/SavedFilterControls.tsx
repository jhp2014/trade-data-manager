...
//
// 저장 단위는 **필터 전체 스냅샷**(밴드+날짜+시간+그룹)이다. 예전엔 밴드만 담아서 불러와도 저장한
...
export function SavedFilterControls({ axes }: { axes: { id: string; name: string }[] }): JSX.Element {
    ...
    const active = Object.keys(rankBands).length > 0 || Object.keys(axisValueRanges).length > 0 || dateRanges.length > 0 || timeRanges.length > 0 || !isGroupExprEmpty(groupExpr);
    ...
    const autoLabel = (): string => {
        ...
        if (!isGroupExprEmpty(groupExpr)) parts.push("그룹");
    ...
    return (
        <>
            <button
                onClick={() => { if (!active) return; const n = prompt("저장 필터 이름", autoLabel()); if (n && n.trim()) saveFilter(n.trim(), { bands: rankBands, axisValueRanges, dateRanges, timeRanges, groupExpr }); }}
                disabled={!active}
                title={active ? "지금 필터(밴드·값구간·날짜·시간·그룹)를 이름 붙여 저장" : "먼저 필터를 거세요"}
