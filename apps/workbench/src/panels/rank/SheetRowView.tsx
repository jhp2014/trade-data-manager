...
function SheetRowViewImpl({
    ...
    const CELLS: Record<ColKind, (c: Col) => CellRender> = {
        ...
        dif: (c) => {
            const v = difOf(row, (c as { id: string }).id);
            return {
                onClick: () => h.onNav(row),
                title: "두 결과 열의 차(A − B) — 한쪽이라도 값이 없으면 값 없음",
                style: { cursor: "pointer" },
                body: (
                    <span className="tabular" style={{ color: v === null ? "var(--text-tertiary)" : toneColor(v > 0 ? "rise" : v < 0 ? "fall" : null) }}>
                        {v === null ? "—" : fmtOutcomePct(v)}
                    </span>
                ),
            };
        },
