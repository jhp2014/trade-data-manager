...
//
// **N/M 무관 트랙과 컷 필터를 가른다**: 트랙(분당 단면 — 하루 ~390분)은 시선/날짜가 바뀔 때
// 한 번이고, 컷 드래그 중에는 bandSegmentsOf(O(분))만 다시 돈다 — 드래그가 정렬을 못 건드린다.
// 분당 단면은 sectionSeries 공용 캐시에서 온다 — 트랙이 채운 단면을 스크럽·꼬리가 재사용한다(역도 성립).
//
