...
//   · 가운데 — **조건 보드**: 걸린 것 전부가 요약 줄로 서고, 켜기/끄기·삭제·생성이 여기 있다.
//
// ⚠ **값 편집은 이 패널에 없다**(2026-08-29 재편). 종류마다 제일 잘 보여주는 편집면이 따로 있다 —
// 1차원(축·날짜·시간)과 그룹은 그 자리 팝오버, 2차원(결과·급타점·테마)만 전용 패널.
...
//
// ⚠ 어휘 — **필터는 과정, 집합은 산출물.** 줄에는 "조건"이, 칩·바인딩·피커에는 "집합"만 보인다.
// 코드의 `stage`(단계)는 아직 core 깔때기의 모델 낱말이라 그대로 둔다 — 2026-09-19 재설계의 식 트리가
// 들어오면 그 자리가 노드가 된다(decisions.md 「집합 편성 재설계」).
