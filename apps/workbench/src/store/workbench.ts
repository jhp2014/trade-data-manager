...
interface WorkbenchState {
    ...
    // 배제 필터(DNF, 그룹별 dim/hide). 술어 = domain 레지스트리. 상태는 보드별 독립, 편집 액션은 공용 팩토리. localStorage 영속.
    ...
    // 배제 필터 편집 — 이슈/복기 각자. 매 변경 localStorage 저장(팩토리 내부).
...
type WorkbenchSet = (fn: (s: WorkbenchState) => Partial<WorkbenchState>) => void;

function loadFilter(key: string): BoardFilterExpr {
    try {
...
function makeFilterActions(set: WorkbenchSet, field: FilterField, persistKey: string): BoardFilterActions {
    const persist = (expr: BoardFilterExpr): void => {
        try {
            localStorage.setItem(persistKey, JSON.stringify(expr));
        } catch {
            /* noop */
        }
    };
    ...
    return {
        ...
        clear: () => set(() => { const next: BoardFilterExpr = { groups: [] }; persist(next); return { [field]: next } as Partial<WorkbenchState>; }),
    };
}
...
export const useWorkbench = create<WorkbenchState>((set) => ({
    ...
    // 배제 필터 편집 — 이슈/복기 각자 상태, 로직 공용 팩토리. 새 그룹=dim 기본, 술어 추가 시 domain 기본 파라미터. 매 변경 localStorage 저장.
