...
describe("저장 집합 — 같은 이름은 엎어쓴다(같은 이름 = 같은 물건)", () => {
    it("첫 저장은 새 항목, 같은 이름 재저장은 id 를 지키고 정의만 갈린다", async () => {
        ...
        const saved = JSON.parse(storage.get("wb.savedSets.v5")!) as { id: string }[];
    ...
    it("옛 키(v4·v3·그 이전)는 읽지 않는다 — 2026-09-20 승계 없는 키 상향", async () => {
        stubStorage({
            ...
            "wb.savedSets": [{ id: "fs3", name: "그 이전", expr: exprOfStages([]), universe: "longitudinal" as const }],
        });
        const store = await loadStore();
...
describe("저장 집합 — 타점 정의 payload (additive, 키 상향 없음)", () => {
    ...
    it("옛 저장물(정의 없음)은 살아남고, 열어도 현재 정의를 유지한다", async () => {
        ...
        const raw = JSON.parse(storage.get("wb.savedSets.v5")!) as Record<string, unknown>[];
        for (const f of raw) {
            delete f.pointDef;
        }
        storage.set("wb.savedSets.v5", JSON.stringify(raw));
