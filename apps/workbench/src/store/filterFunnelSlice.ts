...
//
// 저장 집합(이름 붙인 산출물)은 savedSetsSlice 로 갈라져 있다 — 여기는 **작업 깔때기**(슬롯·시선·선택
// 포인터)만 산다. 두 슬라이스의 접점은 putStages 하나다(깔때기를 만지면 포인터가 복귀하는 규칙).
...
export interface FilterFunnelSlice {
    /** 슬롯 3벌. 이름 없는 고정 칸 — 잠깐 딴 조합을 보는 작업면. 활성 벌은 selectFilterStages 로 읽는다. */
...
/**
 * 활성 슬롯의 단계 리스트 — 소비자(깔때기·보드)는 슬롯을 모르고 이 선택자만 읽는다.
 * 저장 필드가 아니라 **파생**이다: 같은 것을 두 필드로 들면(옛 filterStages) 편집 경로마다 손으로
 * 맞추는 불변식이 생기고, 한 경로만 빠뜨려도 화면과 슬롯이 조용히 갈린다.
 */
export const selectFilterStages = (s: Pick<FilterFunnelSlice, "filterSlots" | "filterSlotIndex">): FilterStage[] =>
    s.filterSlots[s.filterSlotIndex];
...
export const putStages = (
    s: Pick<FilterFunnelSlice, "filterSlots" | "filterSlotIndex" | "funnelSelection">,
    stages: FilterStage[],
): Pick<FilterFunnelSlice, "filterSlots" | "funnelSelection" | "selectedSetRef"> => {
    ...
    return { filterSlots: slots, funnelSelection: keep ? sel : null, selectedSetRef: null };
};
...
export const createFilterFunnelSlice: StateCreator<WorkbenchState, [], [], FilterFunnelSlice> = (set) => ({
    ...
    setFilterSlot: (i) => set((s) => {
        ...
        // 조건 통째 교체 — 시선·선택 포인터·열린 집합(savedSetsSlice) 전부 푼다(다른 깔때기의 것들이라서).
        return { filterSlotIndex: i, funnelSelection: null, selectedSetRef: null, openedSetId: null };
    }),
