...
//  · 필터 = 차원들의 AND. 차원 = 축별 밴드 / 날짜 구간(OR) / 시간 구간(OR) / 태그식(DNF).
//    - 밴드 = 축마다 슬롯 앵커 경계 lo/hi(slotId). 한쪽만 = 반열림, 둘 다 = 구간. reindex 무해.
//    - 날짜/시간 = 구간 배열, 배열 안은 OR(아무 구간에 들면 통과). 빈 배열 = 그 차원 무제한(전체).
//    - 태그 = DNF(그룹 OR / 그룹 안 AND / 리터럴 부정). 빈 식 = 그 차원 무제한. 규칙은 panels/rank/tagFilter.
//  · horizon/bucket = 분석(경로) 파라미터. 임시 질의 상태라 영속 안 함(세션 한정).
import type { StateCreator } from "zustand";
import type { WorkbenchState } from "./workbench.js";
import { EMPTY_TAG_EXPR, type TagExpr } from "../panels/rank/tagFilter.js";
...
export interface RankFilterSlice {
    ...
    tagExpr: TagExpr; // 태그 DNF(빈 식 = 무제한)
    ...
    applyRankFilter: (v: { bands: Record<string, RankBand>; axisValueRanges: Record<string, AxisValueRange[]>; dateRanges: DateRange[]; timeRanges: TimeRange[]; tagExpr: TagExpr }) => void;
    ...
    setTagExpr: (expr: TagExpr) => void; // 식 전체 교체(편집 연산은 순수 tagFilter 가 계산해 넘긴다)
    clearRankFilter: () => void; // 밴드·날짜·시간·태그 전부 해제
...
export const createRankFilterSlice: StateCreator<WorkbenchState, [], [], RankFilterSlice> = (set) => ({
    ...
    applyRankFilter: ({ bands, axisValueRanges, dateRanges, timeRanges, tagExpr }) =>
        set(() => ({ rankBands: { ...bands }, axisValueRanges: { ...axisValueRanges }, dateRanges, timeRanges, tagExpr })),
    ...
    setTagExpr: (expr) => set(() => ({ tagExpr: expr })),
    clearRankFilter: () => set(() => ({ rankBands: {}, axisValueRanges: {}, dateRanges: [], timeRanges: [], tagExpr: EMPTY_TAG_EXPR })),
