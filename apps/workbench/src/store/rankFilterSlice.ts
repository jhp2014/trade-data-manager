...
export interface RankFilterSlice {
    ...
    /** 필터 전체 교체(저장 필터 불러오기) — 저장한 그대로 재현되도록 네 차원을 한 번에 갈아끼운다. */
    applyRankFilter: (v: { bands: Record<string, RankBand>; dateRanges: DateRange[]; timeRanges: TimeRange[]; tagExpr: TagExpr }) => void;
...
export const createRankFilterSlice: StateCreator<WorkbenchState, [], [], RankFilterSlice> = (set) => ({
    ...
    applyRankFilter: ({ bands, dateRanges, timeRanges, tagExpr }) =>
        set(() => ({ rankBands: { ...bands }, dateRanges, timeRanges, tagExpr })),
    ...
    clearRankFilter: () => set(() => ({ rankBands: {}, dateRanges: [], timeRanges: [], tagExpr: EMPTY_TAG_EXPR })),
