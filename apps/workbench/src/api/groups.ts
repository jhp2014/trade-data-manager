...
//
// 옛 태그 클라를 흡수했다. 항목은 grain 별 둘이다: 차트(종목, 날짜) · 좌표 라벨(종목, 날짜, 분 —
// 2026-09-09, Point 저장이 아니라 캔들 좌표에 붙은 라벨). 겹침(징검다리)은 받지 않고 멤버십에서 계산한다.
//
// **지목은 이름으로, 이름은 바디에.** id 는 계약을 안 건넌다(로컬 미러와 Supabase 가 각자 발급 →
// 동기화를 건넌 참조가 조용히 다른 행을 가리킨다). 이름을 경로에 안 싣는 건 자유 텍스트여서다.
import type { Group, GroupItemRef, GroupMembership, GroupPointItemRef, PointGroupMembership } from "@trade-data-manager/wire";
import { apiGet, apiPost, apiPatch, apiPut } from "./http.js";

export type { Group, GroupItemRef, GroupMembership, GroupPointItemRef, PointGroupMembership } from "@trade-data-manager/wire";
...
/** 전 좌표 라벨의 멤버십 한 벌 — 항목은 캔들 좌표(종목, 날짜, 분). */
export const fetchPointGroupMemberships = (signal?: AbortSignal): Promise<PointGroupMembership[]> =>
    apiGet<PointGroupMembership[]>("groups/point-members", undefined, signal);

/** 좌표 라벨 넣기(멱등). */
export const attachPointGroup = (group: string, item: GroupPointItemRef): Promise<void> =>
    apiPost("groups/point-members", { group, item }).then(() => undefined);

export const detachPointGroup = (group: string, item: GroupPointItemRef): Promise<void> =>
    apiPost("groups/point-members/remove", { group, item }).then(() => undefined);
