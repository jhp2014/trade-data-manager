...
// 재료가 어디서 오는지(지금: 복제본 테이블 캐시 4개 + 클라 접기)는 여기 사유다.
...
import { allAnchorsQuery, allCommentsQuery, allPointsQuery, groupMembershipsQuery } from "../api/queries.js";
...
export function usePresenceIndex(): PresenceView {
    ...
    const index = useMemo(
        () => buildPresenceIndex(anchorsQ.data ?? [], pointsQ.data ?? [], memberQ.data ?? [], commentsQ.data ?? []),
        [anchorsQ.data, pointsQ.data, memberQ.data, commentsQ.data],
    );
    return useMemo(
        () => ({
            index,
            isLoading: anchorsQ.isLoading || pointsQ.isLoading || memberQ.isLoading || commentsQ.isLoading,
            error: (anchorsQ.error ?? pointsQ.error ?? memberQ.error ?? commentsQ.error) as Error | null,
        }),
        [index, anchorsQ.isLoading, pointsQ.isLoading, memberQ.isLoading, commentsQ.isLoading, anchorsQ.error, pointsQ.error, memberQ.error, commentsQ.error],
    );
}
