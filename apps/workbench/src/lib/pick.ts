// 짚음(pick) — **보는 집합 안을 좁혀 보는 렌즈**. 조건으로 유도된 부분집합이고, 만드는 패널이 여럿이다
...
// 덮어쓴다. 그래서 겹이 셋이다: 바인딩(보는 집합) → 짚음(렌즈) → 선택(손).
//
// ## 층위 규칙 — 하루 항목은 그 차트의 **모든 타점**에 적용된다
// 깔때기가 하루 항목을 타점으로 펼치는 규칙과 같다: 하루 조건은 그날 타점 전부에 같은 값이라 정직한 반복이다.
