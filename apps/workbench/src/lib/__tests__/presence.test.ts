...
//  ② 담는 것은 **사람 편집물뿐**이다 — 타점은 격자 파생물이라 흔적이 아니다(2026-09-01 손 타점 폐지).
//  ③ 필터는 3상 × AND — "골격 있음 ∧ 그룹 없음" 같은 다음 작업 후보 질문이 성립해야 한다.
...
describe("buildPresenceIndex", () => {
    ...
    it("그룹은 하루 층위 하나 — 이름은 dedupe·이름순", () => {
        const idx = buildPresenceIndex(
            [],
            [membership({ groupNames: ["나"] }), membership({ groupNames: ["가", "나"] })],
            [],
        );
        expect(idx.get("005930|2026-08-01")?.dayGroups).toEqual(["가", "나"]);
    });
});

describe("candidateDaysOf — 후보 하루(분석 모수) 파생", () => {
    it("편집물(앵커∪그룹) 있는 날만 — 코멘트만 남긴 날은 제외(기록≠판단, 옛 서버 union 정의)", () => {
        const idx = buildPresenceIndex(
            [anchor({ stockCode: "A" })],
            [membership({ stockCode: "C", groupNames: ["테마"] })],
            [{ stockCode: "D", date: "2026-08-01" }], // 코멘트만 — 후보 아님
        );
        expect(candidateDaysOf(idx).map((c) => c.stockCode)).toEqual(["A", "C"]);
    });
...
describe("matchesPresence — 3상 × AND", () => {
    ...
    it('"골격 있음 ∧ 그룹 없음" — 다음 작업 후보 질문', () => {
        const f = { baseline: "has", "group-day": "not" } as const;
        expect(matchesPresence(day({ marks: new Map([["baseline", 4]]) }), f)).toBe(true);
        expect(matchesPresence(day({ marks: new Map([["baseline", 4]]), dayGroups: ["가"] }), f)).toBe(false);
        expect(matchesPresence(day({ dayGroups: ["가"] }), f)).toBe(false);
    });
...
describe("DNF 편집 연산", () => {
    ...
    it("종류 추가는 늘 has 로 들어오고 다른 절은 안 건드린다", () => {
        const dnf: PresenceDnf = [{ baseline: "has" }, { comment: "not" }];
        expect(addKind(dnf, 0, "group-day")).toEqual([{ baseline: "has", "group-day": "has" }, { comment: "not" }]);
    });

    it("칩 지우기 — 절에 다른 칩이 남으면 절은 산다", () => {
        const dnf: PresenceDnf = [{ baseline: "has", comment: "not" }];
        expect(removeKind(dnf, 0, "comment")).toEqual([{ baseline: "has" }]);
    });
    ...
    it("모든 연산이 원본을 안 건드린다(식은 props 로만 흐른다)", () => {
        const dnf: PresenceDnf = [{ baseline: "has", comment: "not" }];
        toggleKind(dnf, 0, "baseline"); addKind(dnf, 0, "group-day"); removeKind(dnf, 0, "comment"); removeClause(dnf, 0);
        expect(dnf).toEqual([{ baseline: "has", comment: "not" }]);
    });
});
...
describe("matchesPresenceDnf — 절 사이 OR", () => {
    ...
    const commentOnly = day({ comment: true });

    it('"골격∧!그룹" ∨ "그룹" — 어느 절 하나만 맞아도 통과', () => {
        const dnf = [{ baseline: "has", "group-day": "not" }, { "group-day": "has" }] as const;
        expect(matchesPresenceDnf(baselineOnly, dnf)).toBe(true); // 첫 절
        expect(matchesPresenceDnf(groupOnly, dnf)).toBe(true); // 둘째 절
        expect(matchesPresenceDnf(commentOnly, dnf)).toBe(false); // 둘 다 아님
    });

    it("빈 절은 평가에서 제외 — OR 를 무력화하지 않는다", () => {
        const dnf = [{}, { baseline: "has" }] as const;
        expect(matchesPresenceDnf(commentOnly, dnf)).toBe(false); // 빈 절이 있어도 활성 절이 거른다
        expect(matchesPresenceDnf(baselineOnly, dnf)).toBe(true);
    });

    it("활성 절이 하나도 없으면 전부 통과(필터 없음)", () => {
        expect(matchesPresenceDnf(commentOnly, [])).toBe(true);
        expect(matchesPresenceDnf(commentOnly, [{}])).toBe(true);
    });
});
...
describe("parsePresenceDnf — 영속 복원(옛 형식 승계)", () => {
    it("옛 절-하나 Record 는 [절] 로 감싼다", () => {
        expect(parsePresenceDnf({ baseline: "has", comment: "not" })).toEqual([{ baseline: "has", comment: "not" }]);
        expect(parsePresenceDnf({})).toEqual([]); // 옛 빈 필터 = 필터 없음
    });
    it("새 절 목록은 그대로, 모르는 상태값은 버린다", () => {
        expect(parsePresenceDnf([{ baseline: "has" }, { comment: "not", weird: "yes" }])).toEqual([{ baseline: "has" }, { comment: "not" }]);
    });
    it("모르는 **종류**도 버린다 — 폐지된 '타점'·'타점 그룹' 칩은 화면에도 안 서는 유령이라 여기서 정리된다", () => {
        expect(parsePresenceDnf([{ baseline: "has", point: "has" }])).toEqual([{ baseline: "has" }]);
        expect(parsePresenceDnf([{ "group-point": "has" }])).toEqual([]); // 그것뿐이었으면 절째 사라진다
    });
...
describe("PRESENCE_KINDS", () => {
    ...
    it("이름을 가진 종류는 그룹뿐 — 배지의 색 카드가 이걸 보고 선다", () => {
        expect(PRESENCE_KINDS.filter((k) => k.namesOf).map((k) => k.key)).toEqual(["group-day"]);
    });
});
