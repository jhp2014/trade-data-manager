...
import { trailingHighsOf, previousCloseFromDaily, type ByMarket } from "@trade-data-manager/market";
...
export interface DailyContext {
    ...
    rawPrevClose: ByMarket<number | null>; // 원주가 전일종가(시장별)
}
...
export class KiwoomDailyContext implements DailyContextSource {
    ...
    async ensure(code: string, today: string): Promise<void> {
        ...
        try {
            ...
            const prev = previousCloseFromDaily(raw, today);
            this.cache.set(code, {
                date: today,
                ctx: {
                    trailingHighs: trailingHighsOf(past, today),
                    rawPrevClose: { krx: num(prev?.krxClose), un: num(prev?.unClose) },
                },
            });
        } catch {
            // 무해 — 캐시 미기록, 다음 틱에 재시도.
