...
import { isMinuteCollectionComplete, type CompletionScanReader } from "./minuteCompletion.js";
...
export type { CompletionScanReader };
...
export class DerivedCache {
    ...
    private async doBuild(date: string): Promise<DaySnapshotFile> {
        ...
        if (cacheable && (await isMinuteCollectionComplete(this.deps.scan, date, new Set(codes), "day-snapshot"))) {
